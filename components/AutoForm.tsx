...
// terwijl nieuwe foto's pas bij opslaan naar Vercel Blob gaan.
type FotoItem =
  | { kind: "bestaand"; url: string }
  | { kind: "nieuw"; file: File; preview: string };
...
const formulierVanAuto = (a: Auto): FormState => ({
  ...
  apk: a.apk || "Onbekend",
  btw: a.btw || "Marge",
  bekleding: a.bekleding || "Stof",
  kleurExterieur: a.kleurExterieur ?? "",
...
export default function AutoForm({ initial }: { initial?: Auto }) {
  ...
  const [form, setForm] = useState<FormState>(
    initial ? formulierVanAuto(initial) : leegFormulier()
  );
  const [opties, setOpties] = useState<Optie[]>(
    initial?.opties?.length
      ? initial.opties.map((o) => ({ categorie: o.categorie, items: [...o.items] }))
      : STANDAARD_CATEGORIEEN.map((c) => ({ categorie: c, items: [] }))
  );
  const [fotos, setFotos] = useState<FotoItem[]>(
    initial?.fotos?.length ? initial.fotos.map((url) => ({ kind: "bestaand", url })) : []
  );
  ...
  const handleFotos = (files: FileList | null) => {
    ...
    // Reset de input meteen (de File-objecten zitten al in `nieuw`), anders vuurt onChange
    // niet wanneer je later exact hetzelfde bestand opnieuw kiest — dan lijkt de upload kapot.
      ...
      nieuw.map(
        (file) =>
          new Promise<FotoItem>((resolve) => {
            ...
            reader.onload = (e) =>
              resolve({ kind: "nieuw", file, preview: e.target?.result as string });
            reader.readAsDataURL(file);
          })
      )
    ).then((items) => setFotos((prev) => [...prev, ...items]));
  };
  ...
  // Opslaan — zelfde route voor nieuw én bewerken. Bij bewerken sturen we id/slug/standtijd
  // mee, zodat de bestaande auto wordt bijgewerkt i.p.v. een nieuwe aan te maken.
  const slaOp = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      // Upload nieuwe foto's naar Vercel Blob (gedeelde opslag voor admin + website).
      // Bestaande foto's behouden hun URL; de volgorde in de lijst blijft de toonvolgorde,
      // dus de eerste foto blijft de hoofdfoto.
      const heeftNieuw = fotos.some((f) => f.kind === "nieuw");
      const upload = heeftNieuw ? (await import("@vercel/blob/client")).upload : null;
      const kentekenPad =
        form.kenteken.replace(/-/g, "").toUpperCase() || `auto-${initial?.id ?? Date.now()}`;
      ...
      // verkocht_op: bij verkocht behouden we de bestaande datum of zetten we 'm nu;
      // anders weglaten (zodat de website de auto niet als verkocht toont).
      const verkocht_op = form.verkocht
        ? initial?.verkocht_op || new Date().toISOString()
        : undefined;
      ...
      const auto = {
        ...
        // Bewerken: behoud identiteit + standtijd van de bestaande auto.
        ...(initial
          ? { id: initial.id, slug: initial.slug, toegevoegd_op: initial.toegevoegd_op }
          : {}),
  ...
  return (
    <div className="h-screen overflow-y-auto" style={{ backgroundColor: "#f5f5f5" }}>
      ...
      >
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            >
              {isBewerken
                ? `Bewerken — ${initial?.merk} ${initial?.model}`
                : "Nieuwe auto"}
            </h1>
          </div>
          <button
          ...
          >
            {opslaan ? "Opslaan..." : isBewerken ? "Wijzigingen opslaan" : "Auto opslaan"}
          </button>
        </div>
      </div>
      ...
      <form id="auto-form" onSubmit={slaOp} className="max-w-4xl mx-auto px-6 py-8 flex flex-col gap-8">
        ...
        {/* ── FOTO'S ── */}
        <Sectie titel={`Foto's (${fotos.length})`}>
          <div
            ...
            <>
              ...
              <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                {fotos.map((item, i) => {
                  ...
                  return (
                    <div
                      key={item.kind === "bestaand" ? `b-${item.url}` : `n-${i}-${item.file.name}`}
                      draggable
